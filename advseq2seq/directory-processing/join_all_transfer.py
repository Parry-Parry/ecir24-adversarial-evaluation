...
def main(run_dir : str, out_dir : str):
    ...
    tasb = build_lookup(pd.read_csv(join(run_dir, 'normal_tasb.tsv'), sep='\t', index_col=False))
    colbert = build_lookup(pd.read_csv(join(run_dir, 'normal_colbert.tsv'), sep='\t', index_col=False))
    bm25 = build_lookup(pd.read_csv(join(run_dir, 'normal_bm25.tsv'), sep='\t', index_col=False))
